...
describe("TabelaClassificacao", () => {
  ...
  it("renders the classification table header", () => {
    ...
    // Check if the header titles are rendered
  ...
  it("renders the classification rows for each team", () => {
    ...
    // Check if the rows for each team are rendered
    expect(screen.getByText("Team A")).toBeInTheDocument();
    expect(screen.getByText("30")).toBeInTheDocument(); // Points for Team A
    expect(screen.getByText("10")).toBeInTheDocument(); // Victories for Team A
    expect(screen.getByText("40")).toBeInTheDocument(); // Goals Pro for Team A
    expect(screen.getByText("20")).toBeInTheDocument(); // Goals Contra for Team A

    expect(screen.getByText("Team B")).toBeInTheDocument();
    expect(screen.getByText("25")).toBeInTheDocument(); // Points for Team B
    expect(screen.getByText("8")).toBeInTheDocument(); // Victories for Team B
    expect(screen.getByText("35")).toBeInTheDocument(); // Goals Pro for Team B
    expect(screen.getByText("25")).toBeInTheDocument(); // Goals Contra for Team B
  });

  it("renders the team logos", () => {
    renderWithProvider(<TabelaClassificacao />, mockState);

    // Check if the team logos are rendered
