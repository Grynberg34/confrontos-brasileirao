...
it("renders TabelaHeader, TabelaClassificacao, and TabelaJogos when tabela has data", () => {
  // Mock state with required props
  mockState.tabela = {
    tabela: [
      {
        time: "TeamA",
  ...
  // Check for TabelaClassificacao
  expect(screen.getByText("TeamA")).toBeInTheDocument(); // Ensure the team name is displayed

  // Check for TabelaJogos
  expect(screen.getByText("2 x 1")).toBeInTheDocument(); // Ensure the match score is displayed
});

  it("does not render TabelaHeader, TabelaClassificacao, and TabelaJogos when tabela is empty", () => {
    renderWithProvider(<Tabela />, mockState);

    // Check for the absence of TabelaHeader
  ...
  it("dispatches fetchTabela on mount with the correct parameters", () => {
    ...
    // Check that fetchTabela was dispatched with the correct parameters
